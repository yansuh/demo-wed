...
if (accounts.length === 0) {
    accounts = [
        { id: 1, username: 'phongdaotao', password: '123', fullName: 'Trần Văn Đào tạo', role: 'training' },
        { id: 2, username: 'giangvien', password: '123', fullName: 'Nguyễn Thị Giảng', role: 'lecturer' },
        { id: 3, username: 'khoa_cn', password: '123', fullName: 'Khoa CNTT', role: 'faculty', facultyName: 'Công nghệ thông tin' },
        { id: 4, username: 'admin', password: '123', fullName: 'Quản trị hệ thống', role: 'admin' }
    ];
    saveAccounts();
}
if (subjects.length === 0) {
    subjects = [
        { id: 1, code: 'CS101', name: 'Lập trình Web nâng cao', faculty: 'Công nghệ thông tin', credits: 3, lecturer: 'TS. Nguyễn Văn A', description: '', status: 'active' },
        { id: 2, code: 'CS102', name: 'Cơ sở dữ liệu', faculty: 'Công nghệ thông tin', credits: 4, lecturer: 'ThS. Trần Thị B', description: '', status: 'active' },
        { id: 3, code: 'EC101', name: 'Kinh tế vi mô', faculty: 'Kinh tế', credits: 3, lecturer: 'PGS. Lê Văn C', description: '', status: 'active' },
        { id: 4, code: 'ENG101', name: 'Tiếng Anh chuyên ngành', faculty: 'Ngoại ngữ', credits: 2, lecturer: 'Cô Sarah Johnson', description: '', status: 'active' },
        { id: 5, code: 'CS301', name: 'Trí tuệ nhân tạo', faculty: 'Công nghệ thông tin', credits: 3, lecturer: 'GS. Phạm Minh D', description: '', status: 'active' }
    ];
    saveSubjects();
}
if (logs.length === 0) {
...
app.post('/api/login', (req, res) => {
    ...
    if (user) {
        res.json({ success: true, user: { id: user.id, username: user.username, fullName: user.fullName, role: user.role, facultyName: user.facultyName } });
    } else {
...
app.post('/api/register', (req, res) => {
    const { username, password, fullName, role, facultyName } = req.body;
...
app.get('/api/users', (req, res) => {
    const safeUsers = accounts.map(({ id, username, fullName, role, facultyName }) => ({ id, username, fullName, role, facultyName }));
    res.json(safeUsers);
});
